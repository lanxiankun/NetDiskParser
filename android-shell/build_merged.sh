...
ROOT=/home/user/Doubao/chats/38441058725195778

cd $ROOT/netdisk-parser-app
...
javac -source 8 -target 8 -classpath $PLATFORM -d build/classes \
  ...
  build/gen/com/netdisk/parser/R.java 2>&1 | grep -E "error" || true
echo "[4/8] javac OK"
...
root = "/home/user/Doubao/chats/38441058725195778/android-shell"
...
add_so(root + "/../netdisk-parser-app/libnetdiskparser.so", "lib/arm64-v8a/libnetdiskparser.so")
...
$BT/apksigner sign --ks build/debug.keystore --ks-pass pass:android --key-pass pass:android \
  --out build/NetDiskParser-android-arm64.apk build/app-aligned.apk
$BT/apksigner verify build/NetDiskParser-android-arm64.apk
echo "[8/8] SIGN OK"
cp build/NetDiskParser-android-arm64.apk $ROOT/dist/
ls -la $ROOT/dist/NetDiskParser-android-arm64.apk
