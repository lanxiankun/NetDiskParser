...
// 安卓版日志：写入 logcat（adb logcat -s NetDiskParser 查看）
package main
...
static void logcat_info(const char* msg) {
    __android_log_print(ANDROID_LOG_INFO, "NetDiskParser", "%s", msg);
}
static void logcat_error(const char* msg) {
    __android_log_print(ANDROID_LOG_ERROR, "NetDiskParser", "%s", msg);
}
*/
import "C"
